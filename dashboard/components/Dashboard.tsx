...
// ΓöÇΓöÇΓöÇ SAVINGS SUMMARY CARD ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ ACTIVITY FEED ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ PAGES ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function OverviewPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      {/* Savings Summary Card ΓÇö shown first */}
...
// ΓöÇΓöÇΓöÇ COST ANALYSIS WITH PER-USER DROPDOWN ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {calls.map((c, i) => {
        ...
        const icon = isBlocked ? "≡ƒö┤" : isRouted ? "ΓÜí" : isCached ? "≡ƒÆ╛" : "Γ£ô";
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{ fontSize: 11, color: COLORS.text, fontFamily: "monospace" }}>
              {isRouted ? (
                <span>{c.model_requested} <span style={{ color: COLORS.textDim }}>ΓåÆ</span> <span style={{ color: COLORS.green }}>{c.model_used}</span></span>
              ) : (
            ...
            <div style={{ fontSize: 11, color: color, fontFamily: "monospace" }}>
              {isBlocked ? "ΓÇö" : `$${c.cost_usd.toFixed(6)}`}
            </div>
...
function CostAnalysisPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    <div style={{ background: `${COLORS.green}10`, border: `1px solid ${COLORS.green}30`, borderRadius: 10, padding: 14 }}>
                      <div style={{ fontSize: 10, color: COLORS.textDim, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>≡ƒÆ░ ROI ΓÇö What TokenGuard saved</div>
      ...
      {/* ΓÜí Routing Playground */}
      <Card>
        <CardBody>
          <div style={{ marginBottom: 16 }}>
            <SectionHeader title="ΓÜí Routing Playground" subtitle="Type any prompt ΓÇö see exactly how TokenGuard routes it and what you save" />
            <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
              {[
                { label: "≡ƒÆí Simple", prompt: "What is machine learning?", model: "claude-opus-4-6" },
                { label: "≡ƒô¥ Medium", prompt: "What are the key differences between REST and GraphQL?", model: "gpt-4o" },
                { label: "≡ƒºá Complex", prompt: "Write a comprehensive multi-region fault-tolerant distributed system architecture with step by step implementation plan", model: "claude-opus-4-6" },
              ].map((ex, i) => (
                <button key={i}
...
// ΓöÇΓöÇΓöÇ BUDGETS PAGE WITH PER-EMPLOYEE BARS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      <Card>
        <CardBody>
          ...
          {!userBudgetsLoaded ? (
            ...
            <div style={{ color: COLORS.textDim, fontSize: 12 }}>No spend yet ΓÇö budgets reset</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {userBudgets.map((u, i) => {
                  ...
                  <div key={i} style={{ padding: "14px", background: COLORS.bgAccent, borderRadius: 10, border: `1px solid ${u.blocked ? COLORS.red + "40" : COLORS.border}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: COLORS.textDim, marginTop: 2 }}>Daily cap ΓÇö resets midnight</div>
                      </div>
            ...
            <div key={i}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: COLORS.textDim, marginTop: 2 }}>{b.period} cap ΓÇö resets {b.period_key}</div>
                </div>
                <Badge
...
// ΓöÇΓöÇΓöÇ ROI REPORT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      <Card style={{ background: `linear-gradient(135deg, #0D1220 0%, #0a1628 100%)`, border: `1px solid ${COLORS.green}40` }}>
        <CardBody>
          <div style={{ fontSize: 11, color: COLORS.green, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 20 }}>
            Monthly ROI Report ΓÇö {new Date().toLocaleString("en-US", { month: "long", year: "numeric" })}
          </div>
          ...
          <div style={{ marginTop: 20, padding: 16, background: `${COLORS.green}10`, border: `1px solid ${COLORS.green}20`, borderRadius: 8, fontSize: 13, color: COLORS.textMuted, lineHeight: 1.6 }}>
            ≡ƒÆí <strong style={{ color: COLORS.text }}>Every month you get this report.</strong> You paid us ${fee.toFixed(0)}. We saved you ${savings.toFixed(2)}. That's ${Math.abs(net).toFixed(2)} {net >= 0 ? "net in your pocket" : "net cost after savings"}.
        ...
        <CardBody>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              ...
              { label: "Cache savings", value: billing?.financials?.savings_usd * 0.1 || cacheSaved, desc: `${billing?.usage?.cache_hits || 0} responses served from cache ΓÇö free`, color: COLORS.cyan },
    ...
    <Card>
      <CardBody>
        <SectionHeader
          ...
          subtitle="Full admin access ΓÇö never share this"
        />
        <div style={{
          background: COLORS.bgAccent,
          border: `1px solid ${COLORS.border}`,
    ...
    <Card>
      <CardBody>
        ...
        {loading ? (
          ...
          <div style={{ color: COLORS.textMuted, fontSize: 13, textAlign: "center", padding: 24 }}>
            No employee keys yet ΓÇö click Add Employee to create one
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{
            ...
            {keys.map(k => (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: COLORS.textDim }}>
                  {k.created_at ? new Date(k.created_at).toLocaleDateString() : "ΓÇö"}
                </div>
                <div>
                  <Badge
                ...
                <div style={{ fontSize: 12, color: COLORS.primary, cursor: "pointer", borderBottom: `1px dashed ${COLORS.primary}` }}
                  ...
                  {editBudgetId === k.id ? (
                    ...
                    <span>${k.budget_usd ? Number(k.budget_usd).toFixed(2) : "ΓÇö"}</span>
                  )}
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  >
                    {seedStatus[k.id] === "seeding" ? "..." : seedStatus[k.id] === "done" ? "Γ£ô" : seedStatus[k.id] === "error" ? "Γ£ù" : "Seed"}
                  </button>
                  {k.is_active ? (
                    <button
